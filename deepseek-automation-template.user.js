// ==UserScript==
// @name         DeepSeek Automation Template v2.0
// @namespace    http://tampermonkey.net/
// @version      2.0-template
// @description  DeepSeek automation template — add your prompts and run — Triple-Lock + salvataggio diretto su disco con parti separate
// @author       AutoGen
...
(function() {
  ...
  const PROMPTS = [
    ...
    //
    // End with the marker on the LAST line of your prompt:
    // ===PROMPT_1_COMPLETATO===`
    //   },
    //
    // Rules:
    //   - id: sequential integer starting from 1
    //   - name: short identifier (used for folder & file names)
    //   - content: backtick-delimited template literal
    //   - The marker ===PROMPT_N_COMPLETATO=== MUST be the last
    //     line of each prompt (where N = the prompt id)
    //   - The script uses Triple-Lock detection to confirm the
    //     marker is present in the AI response before exporting
    //
    ...
    //     id: 1,
    //     name: "TOPIC-ONE",
    //     content: `Generate a complete catalog about Topic One.
    //
    // Include sections, tables, and detailed specifications.
    //
    // At the very end, write exactly this marker:
    // ===PROMPT_1_COMPLETATO===`
    //   },
    //   {
    //     id: 2,
    //     name: "TOPIC-TWO",
    //     content: `Generate a complete catalog about Topic Two.
    //
    // Include sections, tables, and detailed specifications.
    //
    // At the very end, write exactly this marker:
  ...
  function createUI() {
    ...
    uiPanel.innerHTML = `
      <div id="figma-auto-header" style="cursor:move;padding:8px 12px;background:#4F46E5;color:#FFF;font-weight:700;font-size:13px;border-radius:8px 8px 0 0;display:flex;justify-content:space-between;align-items:center;">
        <span>\u{1F3A8} Figma Auto v2.0</span>
  ...
  setTimeout(async () => {
    createUI();
    log('\u{1F3A8} Figma DeepSeek Automation v2.0 loaded');
    log('\u{1F4CB} ' + PROMPTS.length + ' prompts | Triple-Lock | Parts mode');
